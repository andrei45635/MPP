...
public class ParticipantsDBRepository implements ParticipantsRepository {
    ...
    public List<Participant> filterByAgeType(Type type, AgeGroup ageGroup) {
        ...
        String query = "select p.participantid, p.name, p.age from participants p  INNER JOIN participantstasks pt on p.participantid=pt.participantID INNER JOIN tasks t on pt.taskID1=t.taskid OR pt.taskID2 = t.taskid where t.type=? AND t.agegroup=?";
        Connection connection = jdbCutils.getConnection();
        try (PreparedStatement preparedStatement = connection.prepareStatement(query)) {
